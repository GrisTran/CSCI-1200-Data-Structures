...
const int Half_size = NUM_ELEMENTS_PER_NODE/2;
...
private:
  ...
  void destroy_list();
  void split(Node<T>* &ptr, Node<T>* &next);
  void resize();
...
//Take the elements from the next node to make this node at least 50% full
template <class T>
void UnrolledLL<T>::split(Node<T>* &ptr, Node<T>* &next){
	//While the node elements < half size and the elements in the next node > 0
	while(ptr->num_elements < Half_size && next->num_elements > 0){
		ptr->elements_array[ptr->num_elements] = next->elements_array[0];
		ptr->num_elements++;
		next->num_elements--;
		for(int i = 0 ; i<next->num_elements; i++){
			next->elements_array[i] = next->elements_array[i+1];
		}
	}
	//If we move all of the node from the next one, delete the next one
	if(next->num_elements == 0){
		if(next->next_ == NULL){
			...
			Node<T>* delnode = next;
			next->next_->prev_ = ptr;
			ptr->next_ = next->next_;
			next = ptr->next_;
			delete delnode;
		}
	}
...
//Merge 2 list into 1
template <class T>
void UnrolledLL<T>::merge(Node<T>* ptr){
	int move_i = ptr->prev_->num_elements;
	for(int i = 0; i<ptr->num_elements; i++){
		ptr->prev_->elements_array[move_i+i] = ptr->elements_array[i];
		ptr->prev_->num_elements++;
	}
...
//Check if any node in the list is not at least 50% full
template <class T>
void UnrolledLL<T>::resize(){
	Node<T>* current = head_;
	...
	while(next != NULL){
		...
		else if(!current->isHalf() && current->next_->isHalf()){
			split(current, next);
		}
	}
}
