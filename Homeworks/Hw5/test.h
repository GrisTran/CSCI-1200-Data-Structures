...
template <class T>
void UnrolledLL<T>::split(Node<T>* ptr){
	while(ptr->num_elements < Half_size && ptr->next_->num_elements > 0){
		ptr->elements_array[ptr->num_elements] = ptr->next_->elements_array[0];
		ptr->num_elements++;
		ptr->next_->num_elements--;
		for(int i = 0 ; i<ptr->next_->num_elements; i++){
			ptr->next_->elements_array[i] = ptr->next_->elements_array[i+1];
		}
	}
	if(ptr->next_->num_elements == 0){
		if(ptr->next_->next_ == NULL){
			Node<T>* delnode = ptr->next_;
			tail_ = ptr;
			delete delnode;
		}
		else{
			Node<T>* delnode = ptr->next_;
			ptr->next_->next_->prev_ = ptr;
